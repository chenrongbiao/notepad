...
struct invoker
{
    //@note 为了减少成员函数封装层数，单独实现了成员函数注册，
    //      其实可以统一走普通函数注册, 只需要将成员函数封装成lamada表达式，就可以执行普通函数注册。
    ...
    call(const Function &f, ThisType *thisType, const std::tuple<Args...> &tp, void *result)
    {
        auto r = call_helper(f,
                             thisType,
                             std::make_index_sequence<function_traits<Function>::value>{},
                             tp);
        *(decltype(r) *) result = r;
    }

    template<typename Function, typename ThisType, size_t... I, typename... Args>
    static auto call_helper(const Function &f,
                            ThisType *thisType,
                            const std::index_sequence<I...> &h,
...
public:
    ...
    {
        m_invokeFunctionWapper = {std::bind(&invoker::apply<Function>,
                                            function_any,
                                            std::placeholders::_1,
                                            std::placeholders::_2)};
    }
    ...
    {
        m_invokeFunctionWapper = {std::bind(&invoker::apply<Function, ThisType>,
                                            function,
                                            p,
                                            std::placeholders::_1,
                                            std::placeholders::_2)};
...
inline void REGISTER_METHOD_FUNCTION(ActorProcessor *processor,
                                     const std::string &identify,
                                     Function func,
                                     ClassPointerType thisPointer)
{
    Actor *actor = new Actor();
